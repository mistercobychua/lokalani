...
export default function Listings() {
  ...
  return (
    <ScreenShell appBar={<AppBar title="Mga Listing" role="vendor" helpKey="vendor-listings" />}>
      <div className="px-4 pb-6 pt-4">
        {vendorPosts.length === 0 ? (
          <EmptyState
            emoji="🏷️"
            title="Wala ka pang listing"
            body="Pindutin ang “Mag-list ng Surplus” sa Dashboard para magbenta ng surplus mo ngayon."
          />
        ) : (
          <div className="space-y-3.5">
            {vendorPosts.map((p) => (
              <ListingRow key={p.id} post={p} onSold={() => { markSold(p.id); showToast(`Marked nabenta: ${p.item}`) }} />
            ))}
          </div>
        )}
      </div>
    ...
    <div className={`overflow-hidden rounded-card border bg-white shadow-card ${sold ? 'border-border opacity-75' : 'border-border'}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-3 text-[12.5px] font-semibold">
            <span className="inline-flex items-center gap-1 text-muted">
              <Eye className="size-[15px]" strokeWidth={2.3} /> {post.views} views
            </span>
            <span className="inline-flex items-center gap-1 text-green-deep">
              <Users className="size-[15px]" strokeWidth={2.3} /> {post.reservations.length} reserved
            </span>
          </div>
        </div>
        {sold && (
          <span className="rounded-lg bg-green-deep px-2.5 py-1 text-[12px] font-bold text-white">Nabenta ✓</span>
        )}
      </div>
      ...
      <div className="mx-3.5 mb-3 rounded-2xl bg-surface p-3">
        <div className="mb-1.5 flex items-center gap-1.5 text-[12px] font-bold text-amber-deep">
          <TrendingDown className="size-[14px]" strokeWidth={2.6} /> Dynamic price curve
        </div>
      ...
      {post.reservations.length > 0 && (
        <div className="border-t border-border px-3.5 py-2.5">
          <p className="mb-1.5 text-[12.5px] font-bold text-green-deep">Mga nag-reserve</p>
      ...
      {!sold && (
        <div className="border-t border-border p-3">
          ...
          >
            <Check className="size-5" strokeWidth={2.5} /> Mark as Nabenta / Picked Up
          </button>
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-[11.5px]">
        <Pt label="Dating" value={peso(orig)} tone="text-muted" />
        <Pt label="Ngayon" value={peso(now)} tone="text-green-deep" />
        <Pt label={dropTime} value={peso(close)} tone="text-amber-deep" />
      </div>
      <span className="sr-only">Bababa ang presyo paglapit ng {freshUntil}</span>
    </div>
  )
}
